...
Other things worth knowing: `tsconfig.json` has `strict: false` and handler params are frequently untyped `any` — match the surrounding style rather than adding strictness piecemeal. Styling is **Tailwind 4** (`@tailwindcss/postcss`) in compatibility mode: `globals.css` does `@import "tailwindcss"` + `@config "../../tailwind.config.js"`, so the theme still lives in the JS config; animations come from `tw-animate-css`. The brand color is `primary` `#5E4AE3`. The deprecated tailgrids plugin was removed, but its load-bearing theme is inlined in `tailwind.config.js`: the `screens` override (sm 540 / md 720 / lg 960 / xl 1140 / 2xl 1320, plus `xs` 400 — these replace Tailwind's default breakpoints app-wide), the redefined `shadow-xs/sm/md/xl`, and the `stroke` / `body-color` / `dark` / `black` (#212B36) colors. `container` is an `@utility` in `globals.css` (centred, 16px gutters). The config also pins Tailwind 3's rem-based `fontSize` line-heights, and `globals.css` has a base-layer block restoring v3 defaults (gray-200 borders, gray-400 placeholders, pointer cursor on buttons, white form fields, 1px cell padding) — removing any of these shifts the UI with no build error. v4 syntax to remember: CSS-variable arbitrary values are `w-(--x)` not `w-[--x]`; `outline-hidden` replaced `outline-none`; `outline-N` now also sets a solid style; `space-y-*` puts margin on the *preceding* sibling, so inline labels need `block`. The default palette is v4's (slightly more saturated than v3). daisyUI was removed; submit-button spinners are Lucide `Loader2` with `animate-spin`. Commit messages follow `type: description` (`feat:`, `refactor:`, `fix:`, `chore:`, `style:`).
