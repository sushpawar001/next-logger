...
Other things worth knowing: `tsconfig.json` has `strict: false` and handler params are frequently untyped `any` — match the surrounding style rather than adding strictness piecemeal. Tailwind loads daisyUI and tailwindcss-animate alongside the shadcn theme (daisyUI is only still needed for the `loading loading-spinner` submit-button spinners, 17 usages across 11 files); the brand color is `primary` `#5E4AE3`. Commit messages follow `type: description` (`feat:`, `refactor:`, `fix:`, `chore:`, `style:`).
