...
### Frontend patterns
- Pages are `"use client"` and fetch their own data with `axios` against the local API; state stays in `useState` (no global store or react-query).
